...
namespace Player.Components.MovementComponents.Utility.Animations
{
    public class FootIKSmooth : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            FootRight = anim.GetBoneTransform(HumanBodyBones.RightFoot).transform;
            FootLeft = anim.GetBoneTransform(HumanBodyBones.LeftFoot).transform;
        }
        ...
        void OnAnimatorIK(int _layerIndex)
        {
            if(IkActive)
            {
                ...
                if (Physics.Raycast(FootPos + Vector3.up, Vector3.down, out hit, rayDistance, RayMask)) //Throw raycast to down
                {
